...
public class BotProvider extends ContentProvider {
	
	public static final Uri CONTENT_URI = Uri.parse("content://com.resea.androtelebot.databaseprovider/element");
	private static final int ALLROWS = 1;
	private static final int NAME=2;
	private static final int NAME_TO_DELETE=3;
	private static final int NAME_FILTER=4;
	private static final String CALLER_IS_SYNC_ADAPTER = "bot";
	private static final UriMatcher uriMatcher;
	static{
		uriMatcher = new UriMatcher(UriMatcher.NO_MATCH);
		uriMatcher.addURI("com.resea.androtelebot.databaseprovider","element",ALLROWS);
		uriMatcher.addURI("com.resea.androtelebot.databaseprovider","element/list/*", NAME);
		uriMatcher.addURI("com.resea.androtelebot.databaseprovider", "element/lists", NAME_TO_DELETE);
		uriMatcher.addURI("com.resea.androtelebot.databaseprovider","element/filter/*", NAME_FILTER);
	}
	...
	public int delete(Uri uri, String selection, String[] selectionArgs) {
		...
		switch (UriType){
		    case ALLROWS:
			    rowsDeleted = db.delete(botDb.KATA_TABLE,selection,selectionArgs);
			    break;
		    case NAME_TO_DELETE:
			    rowsDeleted = db.delete(botDb.KATA_TABLE,botDb.KATA_ROW+"= '"+selection+"'", selectionArgs);
			    break;
	 	    default :
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		switch(uritype){
		    case ALLROWS:
		    	id = db.insert(botDb.KATA_TABLE, null, values);
		        break;
		    default :
		    	throw new IllegalArgumentException("unKnown uri : "+uri);
		}
	...
	public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
		...
		queryB.setTables(botDb.KATA_TABLE);
		int uriType = uriMatcher.match(uri);
		...
		case NAME:
			...
			queryB.appendWhere(botDb.KATA_ROW+"= '"+uri.getLastPathSegment()+"'");
			break;
		
		case NAME_FILTER:
			
			queryB.appendWhere(botDb.KATA_ROW+" LIKE '%"+uri.getLastPathSegment()+"%'");
			break;
